...
double voigt_profile(double *lambdas, double z, double N, int num_lines, int num_points, double *profile) {
    double *raw_profile = (double *)malloc(num_points * sizeof(double));
    double *multipliers = (double *)malloc(num_lines * sizeof(double));
    double total, velocity;
    int i, j, k;
    ...
    for (i = 0; i < num_points - 2 * 3; i++) {
        profile[i] = 0;
        for (j = i, k = 0; j <= i + 2 * 3; j++, k++) {
...
/* Wrapper function to be used in Python */
void compute_voigt(double *lambdas, double z, double N, int num_lines, int num_points, double *result_profile) {
    voigt_profile(lambdas, z, N, num_lines, num_points, result_profile);
}
